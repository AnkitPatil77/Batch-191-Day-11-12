...
public class Stock {
	...
	public void readStocks() {
		...
		try {
			...
			while ((content = bfr.readLine()) != null) {
				...
				stockList[numOfShares].setValue(Double.parseDouble(content.split(" ")[1]));
				stockList[numOfShares].setQuantity(Integer.parseInt(content.split(" ")[2]));
				numOfShares++;
			}
	...
	public void printAllSharesDetails() {
		for (int i = 0; i < numOfShares; i++) {
			...
			System.out.println(share.getShareName() + "\t" + share.getValue() + "\t" + share.getQuantity());
		}
	}
	...
	public void createPortFolioReport() {
		...
		for (int i = 0; i < numOfShares; i++) {
			...
			System.out.println(share.getShareName() + "\tINR " + share.getValue() + "\t" + share.getQuantity()
					+ "\t INR " + totalShareValue);
		}
		System.out.println("---------------------------------------------");
